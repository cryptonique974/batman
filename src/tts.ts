...
const PIPER_MODEL_FR = process.env.PIPER_MODEL_FR || path.join(__dirname, '../data/models/piper/fr_FR-tom-medium.onnx');
const PIPER_MODEL_EN = process.env.PIPER_MODEL_EN || path.join(__dirname, '../data/models/piper/en_US-ryan-medium.onnx');
...
function isFrench(text: string): boolean {
  ...
  const frWords = /\b(je|tu|il|elle|nous|vous|ils|elles|le|la|les|un|une|des|du|de|et|est|pas|que|qui|dans|sur|avec|pour|par|mais|ou|donc|car|ni|or|je suis|c'est|n'est|vous|votre|notre|leur|leurs|bonjour|merci|oui|non|peut|aussi|très|plus|bien|tout|fait)\b/i;
